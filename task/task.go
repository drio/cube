...
func (d *Docker) Run() DockerResult {
	...
	// NOTE: This is pretty bad but gets the job done.
	// Allow us to define in the json task a map between the
	// host port and the container port
	// If the json task definition contains a PortBindings that has two values,
	// we use that to map a Host port to a container port.
	// Otherwise, we let docker assign a port to map to the container port.
	// Notice that in that last mode, docker uses "ExposedPorts" as a the container port.
	if len(d.Config.PortBindings) == 2 {
		containerPort, err := nat.NewPort("tcp", d.Config.PortBindings[1])
		if err != nil {
		...
		hc.PortBindings = nat.PortMap{
			containerPort: []nat.PortBinding{
				{
					HostIP:   "0.0.0.0",
					HostPort: d.Config.PortBindings[0],
				},
			},
		}
	} else {
